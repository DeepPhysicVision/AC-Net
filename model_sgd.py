...
class Capsule(Layer):
    ...
    def call(self, u_vecs):
        ...
        b = K.zeros_like(u_hat_vecs[:, :, :, 0])  # shape = [None, num_capsule, input_num_capsule] 32*82 初始化为0
        ...
        for i in range(self.routings):
            ...
            outputs = self.activation(K.batch_dot(c, u_hat_vecs, [2, 2]))   #更新的权重*原始的胶囊  [?*32*[82]]*[?*32*[82]*128=[?*32*128]深层更新参数
            if i < self.routings - 1:
                b = K.batch_dot(outputs, u_hat_vecs, [2, 3])    #[..*32*[128]] [?*32*82*[128]]=[32*82] 动态路由
        return outputs
    def compute_output_shape(self, input_shape):
...
def build_model(max_len, aspect_max_len, embedding_matrix=[],position_embedding_matrix=[],class_num=3, num_words=10000):
    ...
    multi_aspect = Attention(4,32)([aspect_input_embedding,aspect_input_embedding,aspect_input_embedding]) #*9*64
    ...
    capsule = Capsule(num_capsule=82, dim_capsule=128, routings=4, share_weights=True)(temp) #n*82*64
...
def train_model(sentence_inputs=[], position_inputs=[], aspect_input=[], labels=[], model=None):
    model.fit({'sentence_input': sentence_inputs, 'position_input': position_inputs, 'aspect_input': aspect_input}, labels, epochs=2, batch_size=32, verbose=2)
    return model


def get_predict(sentence_inputs=[], position_inputs=[], aspect_input=[], model=None):
    results = model.predict({'sentence_input': sentence_inputs, 'position_input': position_inputs, 'aspect_input': aspect_input}, batch_size=32, verbose=0)
    return results
